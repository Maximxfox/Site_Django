...
df = pd.read_csv('csv_data/updated_vacancies.csv')
...
plt.grid(True)
plt.tight_layout()
plt.savefig('relevance/salary.png')


vacancies_by_year = df.groupby('published_at')['name'].count()
plt.figure(figsize=(15, 10))
...
plt.grid(True)
plt.tight_layout()
plt.savefig('relevance/count.png')


salary_by_city = df.groupby('area_name')['salary'].mean()
salary_by_city = salary_by_city.sort_values(ascending=False)[:100]
plt.figure(figsize=(15, 10))
...
vacancies_share_by_city = vacancies_by_city / total_vacancies
vacancies_share_by_city = vacancies_share_by_city.sort_values(ascending=False)[:100]
plt.figure(figsize=(15, 10))
vacancies_share_by_city.plot(kind='bar', color='g')
...
skills_by_year = df
skills_by_year['key_skills'] = skills_by_year['key_skills'].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else np.nan)
